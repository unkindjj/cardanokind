...
module Cardano.Api.DeserialiseAnyOf
  ...
  , readAndDeserialiseFromFileAnyOf
  , readAndDeserialiseFromTextEnvelopeFile
  ) where
...
renderInputDecodeError err =
  case err of
    ...
    InputInvalidError -> "Invalid input."
...
readAndDeserialiseFromFile asType acceptedFormats path =
  runExceptT $ do
...
readAndDeserialiseFromTextEnvelopeFile
  ...
  -> FilePath
  -> IO (Either (FileError InputDecodeError) a)
readAndDeserialiseFromTextEnvelopeFile asType fp =
...
readAndDeserialiseFromFileAnyOf bech32Types textEnvTypes path =
  runExceptT $ do
    content <- handleIOExceptT (FileIOError path) $ BS.readFile path
    firstExceptT (FileError path) $ hoistEither $
